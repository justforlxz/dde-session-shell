...
void TimeWidget::refreshTime()
{
    if (m_use24HourFormat) {
        m_timeLabel->setText(m_locale.toString(QDateTime::currentDateTime(), m_shortTimeFormat));
    } else {
        m_timeLabel->setText(m_locale.toString(QDateTime::currentDateTime(), m_shortTimeFormat.append(" ap")));
    }
